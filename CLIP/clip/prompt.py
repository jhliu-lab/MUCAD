...
class EPrompt(nn.Module):
    ...
    def set_prompt(self,saved_prompt, save_status=False):
        if save_status:
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        x_embed=x_embed.to(dtype=torch.float32) # 输入图像
        if prompt_mask is not None: # 提示掩码
            prompt_mask = prompt_mask.to(dtype=torch.float32)
        if cls_features is not None: # 类别特征
            cls_features  = cls_features.to(dtype=torch.float32)
        out = dict()
        if self.prompt_pool:# 若使用提示池，则通过embedding_key与prompt_key的相似度来获取对应的提示
            if self.embedding_key == 'mean': # 均值
