...
class PatchCore(torch.nn.Module):
    ...
    def _embed_prompt(self, images, detach=True, provide_patch_shapes=False):
        ...
        with torch.no_grad():
            # features = self.forward_modules["feature_aggregator"](images)
            # 获取训练数据的特征features shape = (bs, patch_size, patch_size, c)
            features = self.prompt_model(images)['seg_feat']
            for i in range(len(features)):
                # features[i] shape = (1,768, 14,14)
                features[i] = features[i].reshape(-1,14,14,768).permute(0,3,1,2)
        # features shape = (bs, patch_size*patch_size, c, patchsize, patchsize)
        features = [
            self.patch_maker.patchify(x, return_spatial_info=True) for x in features
        ]
        # patch_shapes shape = (bs, c, patchsize, patchsize)
        patch_shapes = [x[1] for x in features]
        # features shape = (bs, c, patchsize, patchsize)
        features = [x[0] for x in features]
        # ref_num_patches shape = c, patchsize, patchsize
        ref_num_patches = patch_shapes[0]
    ...
    def _embed_train_sam(self, images, detach=True, provide_patch_shapes=False, image_path=None, text_cls_feat=None):
        """Returns feature embeddings for images."""
        res = self.prompt_model(images,task_id=self.dataloader_count, cls_features=None, train=True, image_path=image_path, text_feature=text_cls_feat)
        return res
