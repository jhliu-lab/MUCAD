...
import os
import sys
sys.path.append("/home/xr/ljh/mucad_v2")
import math
import logging
...
from mucad_model import CrossAttention
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            prototype_size=5, feature_layer = [5]):
        """
        Args:
        ...
        self.l_feature = []
        self.g_feature = None
        self.cross_attention= CrossAttention(768, 8, 768, 768)
        ###################################
    ...
    def forward_head(self, res, pre_logits: bool = False, label=None, train=False, task_id=-1, image_path=None, text_feature=None, conv_image_feat=None):
        x = res['x']
        # res['seg_feat'] = x[:,1:,:]
        ...
        #     # 将文本正常特征提示融合进正常图片特征中
        #     # cross attention => text_feature shape=[1, 768] and image_feature shape=[8,196,768]
        #     # text_feature shape = > [8, 198, 768] , res['seg_feat] shape == > [8, 196, 768]
        #     text_feature = torch.stack([text_feature[0]]*196) # shape = 196*768
        #     text_feature = torch.stack([text_feature]*res['seg_feat'][0].shape[0]) # 8*196*768
        #     res['seg_feat'][0], _ = self.cross_attention(text_feature, res['seg_feat'][0], res['seg_feat'][0])
        if conv_image_feat != None:
            # use conv_image_feat to fuse with vit image feature
            res['seg_feat'][0], _ = self.cross_attention(conv_image_feat, res['seg_feat'][0], res['seg_feat'][0])
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False, pre_logits=False, label=None, image_path=None, query_task_id=None, text_feature=None, conv_image_feat=None):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train, query_task_id=query_task_id)
        res = self.forward_head(res, pre_logits, label, train, task_id=task_id, image_path=image_path, text_feature=text_feature, conv_image_feat=conv_image_feat)
